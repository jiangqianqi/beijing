...
Page({
  ...
  jumpEvent: function() {
    wx.navigateTo({
      url: '../spot/spot?province=' + myProvince
    })
  },
  jumpTen: function(e) {
    console.log()
  ...
  exec();
   ...
   onLoad: function (option) {
     ...
     var getAreaName = function(scenic){
       if(scenic){
          nowOrScenicUrl = 'https://bjwx.91weather.com/bjwx/app/scenic/realtime?scenicId='+idgitude;
       }else{
            nowOrScenicUrl = 'https://bjwx.91weather.com/bjwx/app/realtime?lon='+longitude+'&lat='+latitude;
       }
       wx.request({
          ...
          success: function(res){
            console.log(res)
            loadHidden();
            alermArea = res.data.regeocode.addressComponent.province;
            ...
            wx.request({
              ...
              success: function (res) {
                ...
                if (res && res.data && res.data.detail && res.data.detail.length > 0) {
                  app.globalData.alerts = res.data.detail;
                  that.setData({
                    alermLogo: true
                  });
                } else {
                  app.globalData.alerts = [];
                  that.setData({
                    alermLogo: false
                  });
                }
        ...
        wx.request({
          ...
          success: function(res){
            ...
            var nowWeather = '';
            if (res && res.data && res.data.detail) {
              nowRain = '降水 ' + (res.data.detail.rain && res.data.detail.rain >= 0.1 ? (res.data.detail.rain + 'mm') : '无');
              nowWind = util.handleWind(res.data.detail.windDir, res.data.detail.windLevel);
              nowRh = '湿度 ' + (res.data.detail.rh ? Math.round(res.data.detail.rh) : '- ') + '%';
              htmlBgimage = res.data.detail.backImage || '';
              nowTemp = res.data.detail.temp || '--';
              nowWeather = res.data.detail.weather ? '未来一小时' + res.data.detail.weather : '';
            }
            var clothesElement = [];
            that.setData({
              nowRain: nowRain,
